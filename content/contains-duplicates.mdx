...
We can leverage the fact that sets only accept unique numbers and don't allow duplicates. If we add all the items from the list 
to the set, and none of the item was removed from the set, it means that the length of the set must be equal to the length of the array.

**Algorithm**

1. Create a set with the input numbers.
2. Compare the length of the array to the size of the set.
3. If the length of the array is not equal to the size of the set, it means that duplicates were removed from the set, return True, otherwise, return False.
