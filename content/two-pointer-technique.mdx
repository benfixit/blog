...
1. Start left at the first index 0, and right at the last index `len(input) - 1`.
2. Using a while loop, perform a logic depending on the problem you are trying to solve and increment left and|or decrement right until they both become equal.
...
### Concluding Remarks:
We've seen how we can use the two pointer technique to optimize the solution to certain types of problems. 
It's very important to see the techniques we applied as guidelines rather than rigid rules. 
A different problem might require a slight variation in the techniques we applied, for example, the [three-sum problem](https://leetcode.com/problems/3sum/description/), which requires the use of three pointers.